...
// 當時不存在的模組會讓整批 task 建立在幻想的介面上。兩個小函式各自重複是已知且可
// 接受的——格式決策各自屬於自己的 capability，日後 JPG 想在檔名帶場地數也不該去動
// 備份檔名。
//
...
/** jpgExportFileName 的輸入參數。 */
export interface JpgExportFileNameParams {
 ...
 */
export function jpgExportFileName(params: JpgExportFileNameParams): string {
	const date = params.exportedAt.slice(0, 10);
	return `${FILE_NAME_PREFIX}${params.roundNumber}-${date}${FILE_NAME_EXTENSION}`;
}
