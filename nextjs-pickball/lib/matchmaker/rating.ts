...
// 不 import candidates.ts 或 player-roster 模組，避免評分邏輯被消費端選人決策耦合。

import { RATING_D } from "./rating-types";

/**
 * 計算預測勝率。
 * 以分差為輸入，按 Elo 級距公式 E = 1 / (1 + 10^(-(Ra - Rb) / D)) 計算，
 * D = 3.0 時分差 0.5 約 60%、1.0 約 68%、2.0 約 82%、3.0 約 91%（prd.md 6.4.2）。
 * 同場對賽的雙方共用同一個 E：一方為 E、另一方為 1 - E（design Decision 4）。
 *
 * @param ratingA 隊伍 A 的平均評分
 * @param ratingB 隊伍 B 的平均評分
 * @returns 隊伍 A 的預測勝率，範圍 (0, 1)
 */
