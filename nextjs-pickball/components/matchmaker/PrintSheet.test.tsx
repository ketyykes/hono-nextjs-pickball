...
import { PrintSheet, PRINT_COURT_DATA_VALUE, PRINT_SHEET_DATA_VALUE } from "./PrintSheet";
...
describe("PrintSheet", () => {
	...
	it("列印版顯示回合標題與每個場地的球員與比分", () => {
		...
		// 兩個場地各自的場地編號
		expect(screen.getByText(/第 1 場地/)).not.toBeNull();
	...
	it("場地數為 3 時渲染 3 個場地區塊而非只有第一個", () => {
		const scene = buildScene({
			courts: [
				buildCourt({ courtNumber: 1, statusText: "未完成" }),
				buildCourt({ courtNumber: 2, statusText: "未完成" }),
				buildCourt({ courtNumber: 3, statusText: "未完成" }),
			],
		});
		render(<PrintSheet scene={scene} />);
