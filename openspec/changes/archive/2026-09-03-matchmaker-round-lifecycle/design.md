...
6. **apply 階段全數完成（2026-08-24，接續第 5 項）**——§7、§8、§9 已完成並通過兩階段審查，tasks.md 62／62 全數勾選。上方第 5 項所列的前三個已知事項均已處置：外層容器改由 `history.ts` 的 `makeHistoryContainerSchema` 工廠產生（嚴格版與寬鬆版共用 `version` 字面量與 `entries` 欄位名）；`HistoryTeamSchema.players` 的人數約束經裁決**不加**（`match-history` spec 與 test-plan 皆無對應 Scenario／MUST，逕自加上會是 scope creep）；主 spec 的歸屬清單已在建立 hook 的同一個 commit 內同步，改動經逐字核對只有那一行。**留給後續 milestone 的事項：**
   - **`useRoundStore` 目前只匯出 `generateRound`**，未接線 `setTargetScore`／`resetIncompleteMatches`／`submitScore`。這是經 Stage 1 裁決的**刻意範圍**（那四個 Requirement 的「實作位於」只列 `round.ts`，test-plan 對應列全部 tier 為 unit 且指向 `round.test.ts`），非遺漏。連帶結果是 `state.history` 只由 hydrate 寫入、hook 從不呼叫 `writeHistory`，且 `round-storage.ts` 的 `clearRound()`／`clearHistory()` 目前**零消費端**。M5 接線 `submitScore` 時 MUST 一併補上歷史的寫入路徑，屆時再判斷那兩個 clear 函式是否為死程式碼。
   - **重置的跨 store 協調尚未存在**：`storage.ts` 的 `RESET_KEYS` 今天就已包含回合與歷史兩個 key，因此 `useRosterStore.resetRoster()` 會把三個 key 一起移除，但 `useRoundStore` **不知道自己被重置了**——記憶體中的 `round`／`history` 仍在，畫面會出現「名單已清空但上一輪還在」。M5 接重置按鈕時需要一個共用的重置入口或從 roster port 傳入的 reset 訊號。若屆時在 `useRoundStore` 新增 `RESET` action 把 `round` 設回 `null`，write effect 會寫出 `{ version: 1, round: null }` 讓 key 復活（`readRound()` 讀它回 `null`，語意與 key 不存在等價，非資料遺失）；真要求「重置後 key 為 null」才需要比照 `useRosterStore` 補 `skipPersist`。
   - **`generateRound` 的 stale closure 限制**：本函式在 render scope 讀 `state.round`，同一次 render 內連呼兩次會共用舊值，使輪次編號少前進一次（休息結算不受影響——`restSettlements` 帶的是絕對值 patch，重複套用結果相同）。已在 `useRoundStore.ts` 該函式上方留註，UI 端 MUST 確保一次互動只呼叫一次。
   - **三支 store hook 的「write effect 在前、hydrate effect 在後」順序皆無 regression guard**：審查時實測把兩個 effect 對調，測試仍全綠。這是 codebase 層級的既有缺口（`useRosterStore`／`useScoreboardStore` 同樣沒有），非本 change 引入，值得另立整理型 change。
   - **`makePlayer` fixture 已是第 9 份逐字複本**（散落 8 支 `lib/matchmaker/*.test.ts` 與 2 支 hook 測試）。收斂應獨立成整理型 change，不該在功能 change 內夾帶。

7. **Final Code Review 結果（2026-08-24）**——判定 `PASS_WITH_NITS`、**0 個 Blocker**。跨群組 mutation 12 次、存活 2 次；五個失敗 discriminated union 的 `ok: false` 分支逐字同形；`Set` ↔ 陣列轉換、比分 parse、勝方判定、`toRoundMatch` 投影、`signatureIndexOf` 皆確認全 repo 只有一份；`PLAYERS_PER_MATCH` 無任何一處被重寫死（改該常數會讓 6 個檔案共 20 條測試轉紅）。合計 diff 18 檔完全落在 proposal 的 Impact 內，「不動」清單實測零改動。leader 依 Nit 修了一項、其餘記錄於此：
   - **已修**：`history.ts` 的 `AssertFormatCovered` 原本是**零消費端的死匯出**，而其上方註解宣稱它「在需要它是 true 的地方轉紅，補上『新增』這一側原本擋不住的缺口」——Final Review 實測把該型別整個改成 `never`，`tsc` 仍 exit 0，等於那段防護當時並不存在（TypeScript 對「型別別名等於 `never`」本身不報錯，必須有實際使用處）。已在 `history.test.ts` 加上消費點 `const formatCovered: AssertFormatCovered = true;`，再跑同一個 mutation 已能得到 `error TS2322: Type 'true' is not assignable to type 'never'`，註解的承諾自此為真。
   - **`ROUND_FAILURE_CODE` 的命名脫離後續群組確立的模式**：其餘四組皆以函式名命名（`SET_TARGET_SCORE_`／`RESET_INCOMPLETE_MATCHES_`／`VALIDATE_SCORE_`／`SUBMIT_SCORE_`），只有 `createRound` 那組以模組命名。這是 §4 先命名、§5／§6 之後才確立模式的順序造成的。成本落在 M5：UI 讀 `ROUND_FAILURE_CODE` 時不會意識到它專屬 `createRound`（`resetIncompleteMatches` 另有一組，且刻意沿用其中的 `INVALID_COURT_COUNT`）。**建議 M5 接線前更名為 `CREATE_ROUND_FAILURE_CODE`。**
   - **`RoundSchema` 的欄位級約束沒有獨立守衛**：把 `RoundMatch.courtNumber` 放寬為 `z.number()`、`Round.createdAt` 放寬為 `z.string()`，全套測試與 `tsc` 皆綠。`round-storage.ts` 的損壞降級測試把「什麼算損壞」百分之百委派給 `RoundSchema`，而 `round-types.test.ts` 只覆蓋 `roundNumber` 一個欄位的約束。目前 spec 對這些欄位沒有 MUST，補測試會是 scope creep——**M6／M7 若要依賴 `courtNumber` 為正整數或 `createdAt` 可排序，MUST 先補約束與紅燈。**
   - **`HistorySchema` 與 `History` 型別目前零消費端**：真正的寫入端 `round-storage.ts` 手寫 `{ version: 1, entries }` 送進 `writeJSON`，從未經過 `HistorySchema`。工廠化仍有實效（mutation 證實讀取端與 round-trip 都守得住），但「嚴格版 ↔ 寬鬆版共用工廠」只落地了一半。與 `clearRound()`／`clearHistory()` 零消費端同屬一類，**M5 接線 `submitScore` 時一併處理**。
   - **讀取骨架目前全 repo 有三份**（`storage.ts`、`round-storage.ts`、`lib/scoreboard/storage.ts`），`hasLocalStorage` 有兩份。Final Review 獨立複核後維持「另立整理型 change」的建議，理由除了 `storage.ts` 不在本 change 可動清單、搬進 `storage-keys.ts` 會推翻 Decision 8 的葉節點定位之外，還多一條：**兩份骨架有實際行為差異**（`storage.ts` 的警告訊息不帶 key、失敗時呼叫公開的 `clearRoster()`；`round-storage.ts` 帶 key 插值、用私有的 `clearKey()`），收斂會構成無紅燈的行為變更。建議範圍為 `storage.ts` + 新的 `storage-io.ts`，並順帶收斂 `lib/scoreboard/storage.ts` 那一份。
