...
派工單位是 `tasks.md` 的 `§` 群組（章節）：**一個群組派發一個全新的 subagent** 當
Implementer，由它一次做完該群組的所有 task。
組內仍**逐 task 依序**執行 TDD——每一條 task 都先寫失敗測試並在 shell 實際看到紅燈，
再最小實作至綠，再 refactor；紅燈誠實條款（regression guard 誠實標註、
**SHALL NOT 偽造紅燈**）完全不變。
兩階段審查改為**逐組**：整組全部完成後才跑 Stage 1（spec 合規，審整組）
與 Stage 2（程式品質，審整組）。
全部群組完成後再派一個 Final Code Reviewer 檢查跨群組整合。
**群組之間 SHALL 嚴格序列，SHALL NOT 平行派發多個 Implementer**——
它們共用同一個 worktree，平行寫會互相覆蓋。
...
| 項目 | 內容 |
|---|---|
| 任務全文 | `tasks.md` 中該群組**所有** task 的 RED + GREEN（或 REFACTOR）配對**完整文字**，含群組標題與 `Depends on:` |
| test-plan 對應列 | `test-plan.md` 中該群組**所有**相關測試那幾列的全部欄位：Test name／Scenario／Assertion／Why first／Tier |
| spec 片段 | `specs/data-transfer/spec.md` 中**該群組觸及的每一條 Requirement** 全文（含其全部 Scenario 與「驗收」錨點） |
| design 片段 | `design.md` 中影響該群組的 Decision 與 Risk 條目（例如 §5 給 Decision 9、§7 給 Decision 2 與 5） |
| §0 的對齊記錄 | `tasks.md` §0 產出的「M4 實際 schema／key 名稱對照表」——§2 以後的群組一律需要 |
...
| 明確的「不給」清單 | 其他群組的內容、其他 capability 的 spec、`prd.md` 全文、其他並行 milestone（M6／M7／M9）的任何檔案 |

Implementer 的執行指示固定為：**依 `tasks.md` 的順序逐條完成該群組的每一條 task**，
每一條都走**寫 RED 測試 → 跑測試、實際看到紅燈並貼出輸出 → 寫最小 GREEN 實作
→ 跑測試看到綠燈 → commit**；整組全部完成後再**自我複查**並一次回報。
...
- **upgrade_to_sonnet_when**:
  - 群組涉及 `csv.ts` 的引號狀態機（引號內的換行與逗號、兩個雙引號的跳脫還原）——
    這是本 change 唯一有真實演算法分支的地方；
  - 群組需要同時協調三個以上既有模組的匯出（例如 §7 的 `CLEAR_ALL_KEYS` 要從
    `lib/matchmaker/storage-keys.ts` 與 `lib/scoreboard/` 下的每一個 key 常數模組
    逐一 import，數量以 §0.5 的 grep 結果為準）；
  - 同一個群組被 Stage 1 或 Stage 2 退回 **1 次**後的重試。
- **upgrade_to_opus_when**:
  - 同一個群組被同一位 reviewer 連續退回 **3 次**（見 Escalation）；
  - 群組回報 `BLOCKED` 且原因為「推理不足」而非「context 不足」；
  - §0 的介面對齊發現 M4 的實際 schema 與 design 假設**結構性不同**（不只是改名），
    需要重新設計 `transfer-types.ts` 的組合方式。
- **checklist（自我複查，整組回報前逐條確認）**:
  - [ ] 該組**每一條** task 都有貼出紅燈輸出；若為 regression guard，有貼出 mutation 的紅／綠輸出
  - [ ] `it` 名稱與 test-plan 的 Test name **逐字相同**（含全形標點）
  - [ ] 只動了該群組任務指定的檔案；未編輯 `storage.ts`／`roster.ts`／`types.ts`／`colors.ts`
...
- **required first action**: 先**複述**自己收到的 spec／test-plan 片段標題。
  若與待審群組對不上，回報 `NEEDS_CONTEXT`，**SHALL NOT 硬審**。
- **checklist**:
  - [ ] 該群組觸及的**每一條** Requirement 的每一條 SHALL／MUST 都有對應的實作或明確的不適用理由
  - [ ] 每一條 RED 測試確實對應到 spec 的那個 `#### Scenario`（不是相近的另一個）
...
- **default_model**: `opus`
- **rationale**: 九個群組由九批不同的 subagent 完成，跨群組的一致性（錯誤訊息語氣、
  結果物件形狀、`Result` 型別的命名）沒有任何單一群組的 reviewer 看得到全貌。
...
- **SHALL NOT**: 重審單一群組的細節（Stage 1／2 已涵蓋）。
...
| 情況 | 處置 |
|---|---|
| 同一位 reviewer 連續退回同一個群組 **3 次** | 升級 Implementer 的模型（haiku → sonnet → opus）後重新派工，**SHALL NOT 以相同模型、相同條件重派** |
| Implementer 回報 `NEEDS_CONTEXT` | 補上缺的 context（多半是 §0 的對齊記錄或 design Decision），以**相同模型**重派 |
| Implementer 回報 `BLOCKED` | 依原因分流：context 不足 → 補 context；推理不足 → 升級模型；群組過大 → 依 task 邊界拆成兩批派工；計畫本身有誤 → 升級給人 |
