...
派工單位是 tasks.md 的 `§` 群組（章節）：**一個群組派一位全新的 Implementer subagent**，一次做完該組所有 task。subagent **不繼承**主對話的任何脈絡，因此下方「Per-group contract」列出的內容必須由 orchestrator **逐字貼入** dispatch 訊息，SHALL NOT 只給檔案路徑要 subagent 自己去讀 plan 檔（schema 的 Forbidden 明列此項）。

**組內仍逐 task 依序執行 TDD**：每個 task 先寫失敗測試並在 shell **實際看到紅燈**，再最小實作至綠，再 refactor，做完一個才進下一個，SHALL NOT 把整組測試一次寫完再一次實作。紅燈誠實條款不變——紅燈若無法自然出現（既有實作已滿足），MUST 在 tasks.md 誠實標註為 regression guard 並以 mutation 驗證證明測試有偵測力，SHALL NOT 改斷言偽造紅燈。

**兩階段審查改為逐組**：整組所有 task 完成後才跑 Stage 1（spec 合規，審整組）→ Stage 2（程式碼品質，審整組），SHALL NOT 在單一 task 完成時觸發。

同一時間只派一位 Implementer，**群組之間仍嚴格序列**，SHALL NOT 平行派工——本段的任務高度集中在 `lib/scoreboard/`、`hooks/useScoreboardStore.ts` 與 M5 的場地區塊元件，平行必然互撞。
...
每位 Implementer subagent 收到且**只**收到下列內容（範圍為**該群組整組**，而非單一 task）：

1. **該組所有 task 的完整文字**（tasks.md 該 `§` 章節底下的**每一組** RED + GREEN 配對，含 REFACTOR 若有），逐字複製、依原順序排列，不摘要、不略過任何一項。
2. **該組所有相關的 test-plan.md 列**：逐列給出 Test name（**逐字**，即 spec 的驗收錨點）／Scenario／Assertion／Why first／Tier。
3. **該組觸及的所有 spec.md Requirement 全文**＋其 Scenarios：**只**給該組觸及的 capability，四個 capability 的 delta 不得整包貼給同一位。觸及 M4／M5 既有測試的兩組（§6 的 `player-roster` 與 §8 的 `match-stage`）MUST 一併附上該 Requirement 的 **MODIFIED 全文**與 design Decision 7，讓 Implementer 知道自己是在**改寫**既有行為而非追加。
4. **相關的 design.md 段落**：影響該組任一 task 的 Decision 與 Risk（例如組內有動 `reducer.ts` 的 task 必附 Decision 6；有動 storage 的必附 Decision 4；有動 `page.tsx` 的必附 Decision 3 含「先讀 `node_modules/next/dist/docs/`」那句）。
...
7. **TDD 三步的硬性要求**（**組內每個 task 各走一輪**）：先寫測試 → 在 shell **實際看到紅燈並貼出輸出** → 最小實作至綠 → refactor → 自我 review → commit，完成一個 task 才進下一個。紅燈若無法自然出現（既有實作已滿足），MUST 誠實回報並改以 mutation 驗證證明測試有偵測力，SHALL NOT 改斷言偽造紅燈。
...
- **其他群組**的 task 文字與其他 capability 的 delta。
- 未被該群組觸及的 capability 主 spec（`quiz`、`tour-experience`、`pickleball-guide-page`、`match-allocation`、`site-navbar`、`api-connectivity`）。`player-roster` 只在 §6（清除範圍）這一組給出「重置名單與二次確認」一條 Requirement，其餘群組不給。
- 整份 `prd.md`（只給該群組對應的節次編號與引文）。
...
- **upgrade_to_sonnet_when**:
  - 群組內有 task 觸及 `hooks/useScoreboardStore.ts` 的 effect 順序（既有的 write/read + `hasHydratedRef` + Strict Mode cleanup 是刻意設計，改壞的失敗模式是靜默競態）。
  - 群組內有 task 觸及 M4／M5 的既有檔案（本 worktree 對它們是唯讀理解，改動需要先讀懂上游意圖）。
  - 群組內含 E2E task（跨頁面導覽 + LocalStorage 前置 + 五個 browser project 的差異）。
  - 同一群組因「測試寫得不精確」被退回一次。
...
- **Self-review checklist**（每個 task commit 前自查）：
  - 測試名稱與 test-plan 的 Test name **逐字**相同（含全形符號與空格）。
  - **組內每個 task 的**紅燈輸出都已貼出，或已明確標註為 regression guard 並附 mutation 驗證。
  - 未動到本群組範圍外的檔案。
...
- **時機**：該群組所有 task 完成後，對整組 commits 審一次。
- **必要的第一個動作**：複述自己收到的 spec／test-plan 摘錄之**標題**。若與待審群組不符，回報 `NEEDS_CONTEXT` 而**不要**開始審查。
...
- **default_model**: `opus`
- **時機**：該群組通過 Stage 1 後，對整組 commits 審一次。
...
- **default_model**: `opus`
- **時機**：全部群組完成後，對所有 commits 做一次跨群組檢視。
- **Rationale**: 本段的四個 capability 分屬數個模組但共用同一組不變式（「槽有條目 ⟺ 綁定有效」、「第一隊 ⟷ us」）。單一群組的 reviewer 只看得到自己那一塊，不變式是否在**所有**路徑上都成立，只有整體檢視能回答。
- **Review checklist**：
  - **不變式跨群組成立**：所有建立槽的路徑都在導向前、所有銷毀場次的路徑都清槽、所有寫入都經 `state.matchId` 推導。
  - 隊伍對應（第一隊 ⟷ `us`）是否真的只有一份定義，入口與回填都取用它。
  - 各群組是否各自造了功能重疊的 helper（例如兩份「讀全部槽」）。
...
- **同一位 reviewer 連續退回同一個群組 3 次** → 升級 Implementer 的模型（`haiku` → `sonnet` → `opus`）後重派。SHALL NOT 以相同模型、相同條件重派。
...
- **BLOCKED 的分類與處置**：脈絡不足 → 補脈絡後同模型重派；推理不足 → 升級模型；群組太大 → 拆成更小的派工單位（子群組，必要時退回逐 task）並同步更新 tasks.md 與本檔；計畫本身錯誤 → 升級給人類。
