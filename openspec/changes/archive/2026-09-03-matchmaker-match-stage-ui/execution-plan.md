...
派工單位是 tasks.md 的 **`§` 群組（章節）**：一個群組派**一個全新的 Implementer subagent**，
由它一次做完該組**所有** task。

組內仍**逐 task 依序**走 TDD：每個 task 先寫失敗測試並在 shell 實際看到紅燈（貼出輸出）→
最小實作至綠 → refactor。紅燈誠實條款不變——加入即綠的測試 MUST 在 tasks.md 誠實標註為
regression guard，SHALL NOT 用「改斷言看紅再改回」偽造紅燈。

兩階段審查改為**逐組**：整組所有 task 完成後才跑 Stage 1（規格符合，審整組）→ Stage 2
（程式品質，審整組），SHALL NOT 在組內逐 task 送審。

群組之間**仍嚴格序列**，SHALL NOT 並行派發多個 Implementer——它們共用同一個 worktree，
並行必然互相覆寫。

全部群組完成後再跑一次 Final Code Reviewer 檢查跨群組的一致性（該階段內容完全不變）。
...
subagent **不繼承主對話的任何 context**。每次派一個群組時，下列項目 MUST 逐字貼進 prompt，
SHALL NOT 只給檔案路徑要對方自己去讀（`schema.yaml` 的 Forbidden 明文禁止讓 subagent 直接讀
計畫檔）：

1. **該群組所有 task 的完整文字**：組內每一項的 RED + GREEN 配對全文（REFACTOR task 則貼該項
   全文），依 tasks.md 的原順序排列——組內要照這個順序逐項執行。
2. **test-plan.md 中該組相關的所有表格列**：Test name / Scenario / Assertion / Why first / Tier
   五欄，一字不改。Tier 決定測試放哪一層與用哪個指令跑，不可省略。
3. **相關的 spec.md 片段**：貼本群組觸及的**所有** Requirement 全文及其 Scenario（含「驗收」
   錨點），僅限相關的那些 capability。SHALL NOT 貼整份 spec。
4. **相關的 design.md 片段**：影響本群組的 Decision 與 Risk 條目。例如 §4 必須拿到
   Decision 4（雙打上下排配置），§7 必須拿到 Decision 5（目標分數鎖定）與 Decision 6
   （重用 `nextRadioIndex`）。
5. **明確的「不給你」清單**：其他群組的內容、其他 capability 的 spec、prd.md 全文、
...
7. **本 repo 的固定紀律**（每次都貼，不假設對方記得）：
   - TDD 三步：先寫失敗測試並在 shell 實際看到紅燈（貼出輸出）→ 最小實作至綠 → refactor。
     組內**每一個 task 各走一輪**，SHALL NOT 把整組的測試一次寫完再一起實作。
...
- **upgrade_to_sonnet_when**:
  - 群組為 §11（頁面組裝）——那裡要把八個元件、五個純函式與 M4 的介面接在一起，
    是本 change 唯一需要跨檔案推理的地方。
  - 群組觸及 E2E（`match-stage.spec.ts`、`navbar-rwd.spec.ts`）——Playwright 的等待語意、
    五個 browser project 的差異、boundingBox 量測都容易寫出假綠。
  - 同一個群組被同一個 reviewer 退回 2 次。
- **upgrade_to_opus_when**:
  - 同一個群組被同一個 reviewer 退回 3 次（見 Escalation）。
...
- **self-review checklist**（提交前自己先過一遍）:
  ...
  - [ ] 沒有動到本群組範圍外的檔案；特別是沒有新增任何 `hooks/` 檔案（design Decision 3）。
...
- **同一 reviewer 對同一個群組連續退回 3 次** → 升級 Implementer 的模型
  （`haiku` → `sonnet` → `opus`）後重新派該群組。SHALL NOT 用同一個模型在同樣條件下重試——
  那只會得到同樣的結果。
...
- **本 change 專屬的升級條件**：
  - Implementer 回報「M4 的匯出與 design 假設不符」→ **立即停止該群組**，升級給人類。
