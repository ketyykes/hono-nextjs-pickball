...
- [x] 7.3 REFACTOR: 把四類驗證抽為單一具名函式（例如 `assertValidInput`），訊息模板集中一處；確認驗證在任何計算之前執行（避免先算出 `NaN` 再報錯）
  > **已完成，四條錯誤訊息文字逐字未變、24 passed**。合併為單一 `assertValidInput` 後對資料只走訪一趟；
  > 訊息模板抽為模組層級的 `createValidationError`（人數／`rating`／`gamesPlayed` 三條共用，
  > 重複 id 那條刻意不併入——它的對象是 M4 接線錯誤而非使用者輸入，硬套「請調整後再試一次」會扭曲語意）；
  > `for (let i = 0; i < 2; i++)` 改 for-of；`PLAYERS_PER_MATCH[format] / 2` 由兩處收斂為一處。
  > **步驟順序**：先收緊六處驗證訊息斷言，再搬訊息模板——否則搬動時把兩條訊息互換測不出來。
  >
  > **⚠️ 對 verify 階段的留痕：多重違規時「拋出哪一條」的優先序改變了**。舊版是「四類各掃全部 →
  > 類別優先序固定」，新版是「單趟交錯 → 位置優先序」。例如「單打、A 隊 `rating` 0.99 且 B 隊 2 人」
  > 舊版報隊伍人數、新版報 `rating`。Stage 2 以 HEAD 版與新版各自 bundle 實跑 11 組
  > 「單一違規／合法輸入」逐字比對，輸出完全相同；**拋錯與否的判定集合未變**，spec、test-plan、design
  > 亦未規定優先序，故判為可接受的重構副作用。記錄於此以免 verify 時被當成迴歸重新調查。
