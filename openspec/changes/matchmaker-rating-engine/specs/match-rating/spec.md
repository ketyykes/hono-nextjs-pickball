...
零和 MUST 在下列條件**全部**滿足時成立，且 MUST 以 `rawDelta`（四捨五入**前**的理論變動）衡量：

1. 參與者的 `K_eff` 全部相同（即 `gamesPlayed` 全部相同）；
2. 無人撞上下限。

**守恆是 `rawDelta` 的性質，不是 `delta` 的性質。** `delta` 由兩個各自獨立四捨五入的 `after` 減去 `before` 得到，兩次 round 的誤差沒有互相抵銷的機制——`roundRating` 採 half-up，當雙方的 `rating ± rawDelta` 尾數**同時**落在 `.xx5` 時兩側殘差同號相加，`delta` 之和可**超過** `0.01`。

實測反例：雙方 `rating` 皆為 `5.27`、`gamesPlayed` 皆為 `10`（`E = 0.5`、`K_eff = 0.25`、`rawDelta = ±0.125`）：

```
勝方  5.27 + 0.125 = 5.395 → round → 5.40   delta = +0.13
敗方  5.27 − 0.125 = 5.145 → round → 5.15   delta = −0.12
delta 之和 = 0.01000000000000156       ← 超過 0.01
```

這不是浮點雜訊（雜訊僅佔 `1.5e-15`），是 half-up 的系統性偏差。掃描 5608 組同分同場次組合（`rating` 1.00～8.00 × `gamesPlayed` ∈ {0,5,10,15,20,30,40,80}）有 **474 組（8.45%）** 的 `delta` 殘差超過 `0.01`。

因此驗收 SHALL 以 `rawDelta` 斷言守恆；若要對 `delta` 設界線，容差 MUST 不小於 `0.02`。
...
- **THEN** 勝方與敗方的 `rawDelta` MUST 互為相反數
- **AND** SHALL NOT 以 `delta` 斷言此性質——`delta` 的殘差可超過 `0.01`（見本 Requirement 的 half-up 說明）
