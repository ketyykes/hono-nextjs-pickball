...
- [ ] 3.2 **綠**：新增 `SET_TARGET_SCORE` case（比照 `SET_MODE`：`status !== "setup"` 時 return state，否則重建初始 state 並保留另外兩項設定）。重跑 3.1 指令至綠
...
- [ ] 3.4 **綠**：`createInitialState` 與 `UNDO`／`RESET` 的重建路徑帶入 `targetScore`。重跑 3.1 指令至綠
