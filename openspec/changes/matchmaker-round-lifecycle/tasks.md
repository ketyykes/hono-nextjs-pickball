...
- [x] 8.1 RED: 新增 `nextjs-pickball/hooks/useRoundStore.test.tsx`，寫入 it「重新掛載後還原目前回合與歷史」（tier: integration，走真實 `localStorage`）。確認紅燈。**同時預期 `hooks/hooksInventory.test.ts` 的 it「hooks 目錄下每支 hook 都能在規格的歸屬清單中找到」在 8.2 建立 `useRoundStore.ts` 後轉紅**——這是真紅燈，貼出輸出。**兩條紅燈皆為真紅燈**：① `useRoundStore.test.tsx` 在 `useRoundStore.ts` 不存在時因 import 解析失敗而紅（`Failed to resolve import "./useRoundStore"`）；② 建立 `useRoundStore.ts`（尚未同步主 spec 前）跑 `hooksInventory.test.ts`，「hooks 目錄下每支 hook 都能在規格的歸屬清單中找到」因 `missing: ["useRoundStore"]` 轉紅，同步主 spec 第 141 行後轉綠
- [x] 8.2 GREEN: 新增 `nextjs-pickball/hooks/useRoundStore.ts`：`useReducer` + write effect 在前、hydrate effect 在後、`hasHydratedRef` 守門（沿用 `useRosterStore.ts` 的既有結構）；以 roster port `{ players, updatePlayer }` 注入名單（design Decision 7）；`generateRound` 在**同一次 reducer action** 內套用 `restSettlements` 與新回合（design Decision 1）。**同一個 commit 內**依 `specs/pickleball-guide-page/spec.md` 的 delta，把 `` `useRoundStore` → round-lifecycle `` 一項加入 `openspec/specs/pickleball-guide-page/spec.md` 的歸屬清單——**只准加這一項，該 Requirement 的其他文字一個字都不准動**（design Decision 9）。跑 `hooksInventory.test.ts` 確認兩個既有 it 皆轉綠：「hooks 目錄下每支 hook 都能在規格的歸屬清單中找到」與「歸屬清單提及的每個 hook 名稱都有對應檔案」。**範圍註記**：round-lifecycle delta 中僅「休息次數於產生新一輪時結算」與「回合與歷史的持久化與損壞降級」兩個 Requirement 的「實作位於」列出 `useRoundStore.ts`；`setTargetScore`／`resetIncompleteMatches`／`submitScore` 三個 Requirement 只列 `round.ts`，故本次 `useRoundStore` 只匯出 `generateRound`，其餘三者的 store 接線留待後續（M5 或後續 task）依各自的紅燈流程加入，非本組遺漏。測試前置（GIVEN「已產生一個回合並完成一場對戰」）改為直接呼叫 `round.ts` 的 `createRound`／`submitScore` 與 `round-storage.ts` 的 `writeRound`／`writeHistory` 組出既有持久化資料，不透過 hook——理由同上
- [x] 8.3 REFACTOR: 確認 `useRoundStore` 內沒有任何回合邏輯——狀態轉換一律委派 `round.ts` 的純函式，hook 只負責 dispatch、持久化與把 patch 交給 roster port；確認未在 hook 內呼叫 `useRosterStore()`（會產生第二個名單實例，見 design Decision 7 的否決理由）。自我檢查結果：兩項皆確認成立（見下方 REFACTOR 自查記錄）。另修正一處：`useReducer` 惰性初始化原沿用 `useRosterStore.ts` 的 `(_arg: undefined) => createInitialState()` 寫法會產生新的 `no-unused-vars` warning（`useRosterStore.ts` 已有此既存 warning，但硬規則要求本檔不得新增），改為直接傳入 `createInitialState`（無參數函式，結構相容 `useReducer` 的惰性初始化簽章），`npx eslint` 轉為 0 warning

  **審查紀錄**：Stage 1 PASS——並就「hook 只暴露 `generateRound`」的範圍判斷做出裁決：tasks 8.2 內文與 test-plan 皆未要求接線另三個函式，delta 中那四個 Requirement 的「實作位於」只列 `round.ts`，且 design Decision 7 提到「需要寫 rating／gamesPlayed」屬**說明 roster port 為何存在的理由陳述**而非本 change 的落地要求，故現況正確、非漏做。Stage 1 自行做 8 次 mutation、4 次轉紅（其中「移除 `hasHydratedRef` 守門」轉紅，證明 write／hydrate 競態守門確實被整合測試蓋到）。Stage 2 PASS_WITH_NITS、無 Blocker，自行做 9 次 mutation、**9 次全數存活**，根因單一：本檔唯一的 it 只走 hydrate 路徑，`generateRound` 從未被呼叫。leader 依此補了一條測試與三處註解修正：
  - 新增 it「產生新一輪時把上一輪的休息結算交給 roster port，並以上一輪為基準遞增輪次」——**regression guard 非真紅燈**（行為已成立）。補它的理由是 Stage 2 實測「把 hook 內整段 `restSettlements → updatePlayer` 接線刪光，全套測試仍全綠」：spec「休息次數於產生新一輪時結算」的「實作位於」同時列了 `round.ts` 與本 hook，但其五個 Scenario 的驗收全指向 `round.test.ts`，只驗 `createRound` 回傳值算得對不對，驗不到 hook 有沒有把它套用出去。做法採 design Decision 7 尾段預告的 `updatePlayer` spy。已 mutation 驗證殺得死 Stage 2 五個存活項（刪除接線、`previousRound` 固定 `null`、產生新一輪順手清歷史、reducer 就地修改、write effect 依賴改 `[]`），五次皆轉紅、還原後 2 測試回綠。註：GIVEN 刻意先寫入一筆歷史，否則「順手清歷史」的 bug 會與正確行為得到同樣的空陣列而分辨不出來
  - 註解來源修正兩處：原「（design：`readHistory()` 的 `droppedCount` MUST 能被 hook 消費端看見）」但 `design.md` 通篇無 `droppedCount`，真正出處是 spec 的「丟棄筆數大於 0 時 SHALL 對外回報」，已改指向 spec；原「`round.ts` 明文『呼叫端（也就是你的 hook）注入』」的引號內容全 repo 零命中（`round.ts` 原文為「時間與場次 id 一律由呼叫端注入」），已改為不失真的轉述
  - 補上 `generateRound` 的已知限制註解：本函式在 render scope 讀 `state.round`，同一次 render 內連呼兩次會共用舊值而使輪次編號少前進一次（休息結算不受影響——`restSettlements` 帶的是絕對值 patch，重複套用結果相同）。不搬進 reducer 根治的理由一併寫在該處

## 9. 收尾驗證
Depends on: §1, §2, §3, §4, §5, §6, §7, §8

- [x] 9.1 逐條核對四份 delta spec 的每個「驗收」錨點：檔案路徑存在、it 名稱逐字相符。以腳本抽取 `**驗收**：\`<path>\`，it 名稱「<name>」` 並比對實際 `it("...")`，**不靠目視**。貼出「共找到 N 個錨點 / M 個對上」的輸出。四份 delta 合計 **71 個錨點**，其中 13 個屬於兩個 MODIFIED capability 中**未改動**的 Scenario（`types.test.ts` 6 個、`useScroll*.test.ts` 4 個、`hooksInventory.test.ts` 1 個、`player-roster.spec.ts` 2 個），它們指向本 change 之前就存在的 it，同樣必須逐字對上。**結果**：`共找到 71 個錨點 / 71 個對上`，全部逐字對上（腳本同時支援 `it 名稱` 與 e2e 的 `test 名稱` 兩種措辭——`player-roster.spec.ts` 那 2 個錨點用的是 `test 名稱`，只認 `it 名稱` 的正規表示式會少抓 2 個而得到 69）
- [x] 9.2 spec 條目重複檢查：依 root `CLAUDE.md` 指定的 python 計數法逐標題計數，**不使用 BSD `uniq`**（`sort | uniq -d` 會把內容不同的中文標題誤判為重複）。**結果**：四份 delta 與 `openspec/specs/` 全部 9 份主 spec 逐標題計數皆為「無重複」
- [x] 9.3 `pnpm lint`（repo root）：0 errors。既有的 3 個 warning（`useQuiz.ts`／`useRosterStore.ts`／`useScoreboardStore.ts`）不算新增；`useRoundStore.ts` 若產生新 warning 需處理或說明。**結果**：`✖ 3 problems (0 errors, 3 warnings)`，3 個 warning 全在 `useQuiz.ts`／`useRosterStore.ts`／`useScoreboardStore.ts`，與 baseline 完全相同；`useRoundStore.ts` 與 `lib/matchmaker/` 皆 0 warning
- [x] 9.4 `pnpm typecheck`（repo root）通過。**結果**：exit 0，無任何輸出
- [x] 9.5 `pnpm test`（repo root）全套通過，貼出前後端的檔數與測試數。特別確認 `types.test.ts`、`roster.test.ts`、`storage.test.ts`、`hooksInventory.test.ts` 這四支 M1 既有測試全綠——本 change 動過 `storage.ts` 與主 spec 的歸屬清單。**結果**：前端 `Test Files 46 passed (46)`／`Tests 357 passed (357)`；後端 `Test Files 4 passed (4)`／`Tests 16 passed (16)`。相對 baseline（前端 45 檔／349 測試）為 +1 檔（`useRoundStore.test.tsx`）+8 測試（§7 五條 + round-trip guard 一條、§8 hydrate 一條 + 休息結算 guard 一條）。四支 M1 既有測試單獨重跑亦全綠：`Test Files 4 passed (4)`／`Tests 23 passed (23)`
- [x] 9.6 `pnpm test:e2e` 跑 `tests/e2e/specs/player-roster.spec.ts`：確認重置流程未因 `RESET_KEYS` 擴大而破壞。本 change **不新增任何 e2e**（無 UI）；若沙箱擋住 workerd／miniflare 的 `listen EPERM`，放行後重跑，那不是設定錯誤。**結果**：`20 passed (44.6s)`（4 條 test × chromium／firefox／webkit／mobile-chrome／mobile-safari 五個 project），含「確認重置後名單清空且持久化資料被移除」與「取消重置後名單維持不變」兩條重置流程。跑之前依 root `CLAUDE.md` 的指引先以 `lsof -i :3005 -i :8787` 與 `ps aux` 排查，發現主 repo 於 8/17～8/18 遺留的 `wrangler dev`／`workerd`／`next dev` 共 6 個 process，且 `curl :8787/api/health` 完全無回應（正是該文件記載的「有 process 但後端已壞」情形），全數 kill 並確認 port 釋放後才起單一組。輸出中的 `ChunkLoadError` 為 turbopack dev server 的 HMR chunk 噪音，非測試失敗
- [x] 9.7 `DO_NOT_TRACK=1 openspec validate matchmaker-round-lifecycle --strict`（repo root）通過，貼出輸出。**結果**：`Change 'matchmaker-round-lifecycle' is valid`（exit 0）
- [x] 9.8 確認本 change 的實際改動檔案清單與 proposal 的 Impact 一致，沒有未申報的檔案；`git diff --stat` 中 `openspec/specs/` 的改動 MUST 只有 `pickleball-guide-page/spec.md` 的歸屬清單一行。**結果**：`openspec/specs/` 的改動**只有** `pickleball-guide-page/spec.md` 一個檔、一行、且逐字只在 `` `useRosterStore` → player-roster； `` 之後插入 `` `useRoundStore` → round-lifecycle； ``，符合硬性要求。實作檔共 13 個，與 Impact 的「新增」「修改」逐項對上；`不動` 清單（`roster.ts`／`colors.ts`／`candidates.ts`／`pairing.ts`／`duplication.ts`／`app/matchmaker/**`／`components/matchmaker/**`／`lib/scoreboard/**`）確認零改動。核對時發現 `lib/matchmaker/storage.test.ts`（+7 行，重置範圍斷言擴大為三個 key）雖已列於 test-plan 的「既有測試檔的擴充」，卻未出現在 proposal 的 Impact「修改」清單，已補列一行使兩份 artifact 一致——這是文件補正，非新增改動
