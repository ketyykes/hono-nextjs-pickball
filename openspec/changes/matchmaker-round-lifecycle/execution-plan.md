...
每個 task 派出一個**全新的 subagent**，不共用對話脈絡；同一時間只有一個 Implementer 在跑（禁止並行，會在同一個 worktree 互相踩）。每個 RED／GREEN task 跑完整三角色流程（Implementer → Stage 1 → Stage 2）；REFACTOR task 只跑 Implementer → Stage 2。

## Per-task contract

Subagent **不繼承主對話的任何脈絡**，以下就是它拿到的全部內容。缺任何一項就必須由 orchestrator 補齊後重派，不要讓 subagent 自己去讀計畫檔（會讀到不屬於它的 task 而擴大改動範圍）。

派工時 MUST 提供：

1. **tasks.md 中該 task 的完整原文**——RED 與 GREEN 成對貼出，一字不改（含 it 名稱與括號內的註記）。
2. **test-plan.md 中對應的那一列**——`Test name` / `Scenario` / `Assertion` / `Why first` / `Tier` 五欄全給。`Test name` 就是要寫進 `it(...)` 的字串，逐字使用。
3. **相關的 spec.md 段落**——只給該 task 觸及的那一個 capability 的那一條 Requirement 與其 Scenario，不給整份 spec。
4. **相關的 design.md 段落**——影響該 task 的 Decision 與 Risk 原文。例如所有涉及 `restCount` 的 task 必附 Decision 1；涉及重排的必附 Decision 5；涉及 `submitScore` 的必附 Decision 6；涉及 `useRoundStore` 的必附 Decision 7 與 9。
...
6. **可動檔案清單**——該 task 允許新增或修改的檔案，明確列舉。清單外的檔案一律不得改動；需要改才做得到時回報 `NEEDS_CONTEXT` 或 `BLOCKED`，不要自行擴權。
7. **單檔測試指令原文**：`pnpm --filter ./nextjs-pickball test --run <path>`。**`--run` 前不可加 `--`**——加了 vitest 會收不到路徑而跑完整套，紅燈證據會被既有綠燈淹沒。
8. **明確的「不給」清單**（見下）。

明確**不給**：

- 其他 task 的內容（避免順手實作下一步而讓 Stage 1 判為 scope creep）。
- 與本 task 無關的 capability spec（例如寫 `history.ts` 時不給 `round-lifecycle` 的持久化 Requirement）。
- `overview.md`（人類用摘要，apply 階段不需要，且其 ASCII 圖容易被誤讀為實作規格）。
- `proposal.md` 的完整內容（只在 task 需要判斷範圍邊界時，節錄「不在本次範圍」那一段）。
- 主 spec `openspec/specs/**` 的檔案——**唯一例外**是 §5 那個同步 hooks 歸屬清單的 task，該 task MUST 額外附上 `specs/pickleball-guide-page/spec.md` delta 全文與 design Decision 9，並明確界定「只准新增 `useRoundStore` → round-lifecycle 這一項，其他文字一個字都不准動」。
...
- **upgrade_to_sonnet_when**：
  - task 涉及**跨模組整合**——`submitScore`（§3.13～3.14）、`useRoundStore`（§5）：需要同時抓住回合、名單、歷史三份資料的一致性。
  - task 需要**接住既有模組的例外或轉換型別**——`Set` ↔ `string[]`、接住 `allocateRound` 的 `Error`（§3.3、§3.4）。
  - 同一個 reviewer 連續 2 次退回。
- **upgrade_to_opus_when**：
  - 同一個 reviewer 連續 3 次退回（見 Escalation）。
  - task 被判定為「plan 本身有誤」而非實作有誤（例如 spec 的兩條 Scenario 互相矛盾）。
  - `resetIncompleteMatches`（§3.9～3.10）：候選池重算、場地數扣除與基準合併三件事交纏，是本 change 邏輯密度最高的一段。
- **必做流程**：寫 RED 測試 → 在 shell 實際跑單檔看到紅燈並**貼出輸出** → 寫最小 GREEN 實作 → 再跑看到綠燈 → 自我檢查 → commit。
...
- **必做的第一個動作**：複述它收到的 spec / test-plan 節錄的**標題**。若與被審的 task 對不上，回報 `NEEDS_CONTEXT`，**不要**開始審——由 orchestrator 重組脈絡後重派。
- **檢查清單**：
  - [ ] 該 task 宣稱覆蓋的 Scenario，其 WHEN／THEN 是否逐句都能在測試斷言中找到對應？
  ...
  - [ ] RED 的紅燈證據是否存在？若標為 regression guard，理由是否成立（被守護的行為在此之前是否真的已成立）？
  - [ ] 改動範圍是否落在 task 給的可動檔案清單內？
...
- **不看**：spec 對不對、要不要做這件事——Stage 1 已經處理過，在此重開會讓 task 永遠收不了尾。

### Final Code Reviewer

- **default_model**: `opus`
- **rationale**：跨 task 的整合問題（同一概念兩個名字、兩個模組各寫一份同樣的轉換、合計 diff 超出 proposal 範圍）在單 task 視角下**看不見**，只有把全部 commit 攤開才會顯現。這正是 M2 第 4～6 批 review 抓到 `roundRating` 重複與死碼場地編號的那一類問題。
- **時機**：所有 task 都完成、全部 checkbox 打勾之後，跑一次。
- **檢查清單**：
  - [ ] 命名與模式在 §1～§5 之間是否一致？（例如失敗結果的形狀是否每個函式都用同一個 discriminated union？）
  - [ ] 有沒有兩個 task 各自引入了同一段邏輯（`Set` ↔ 陣列的轉換、比分字串的 parse、勝方判定）？
...
- **不看**：單一 task 的細節——Stage 1／2 已經逐 task 審過。
...
- **Implementer 被同一個 reviewer 連續退回 3 次** → 升級模型（haiku → sonnet → opus）後重派。**SHALL NOT 以相同模型、相同條件重派**——同樣的輸入只會得到同樣的輸出。
