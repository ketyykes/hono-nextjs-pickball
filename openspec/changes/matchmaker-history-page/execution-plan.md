...
每一個 tasks.md 的 checkbox 由一個**全新**的 subagent 執行，執行完依序跑 Stage 1（spec 合規）與 Stage 2（程式品質）兩道審查。所有 subagent 共用 [environment.md](./environment.md) 宣告的**同一個 worktree**，SHALL NOT 各自另開 worktree。

## Per-task contract

subagent **不繼承主對話的上下文**，以下是它拿到的全部資訊。派工時一律**貼全文**，SHALL NOT 只給檔案路徑要它自己去讀計畫檔。

派工必附：

1. **tasks.md 的完整 task 文字**——RED 與 GREEN 成對給（REFACTOR 則單獨給，見下方 REFACTOR 契約）。不得只給其中一半。
2. **test-plan.md 中對應的整列**——Test name／Scenario／Assertion／Why first／Tier 五欄逐字貼上。Tier 決定測試要寫在哪：`unit` → `nextjs-pickball/lib/matchmaker/history-range.test.ts`；`e2e` → `nextjs-pickball/tests/e2e/specs/matchmaker-history.spec.ts`。
3. **spec.md 的相關段落**——`specs/match-history/spec.md` 中該 task 所屬的那一條 `### Requirement:` 全文，含其下所有 `#### Scenario:`。只給這一條，不給整份。
4. **design.md 的相關決策**——影響該 task 的 Decision 與 Risk 全文。對照表：
...
7. **TDD 三步的明確指示**——先寫 RED 測試 → 在 shell 實際跑到紅燈並**貼出輸出** → 寫最小 GREEN 實作 → 重跑至綠 → 自我審查 → commit。若測試加入後**直接是綠的**，MUST 在回報中誠實標註為 regression guard，SHALL NOT 用「改斷言看紅再改回」偽造紅燈（root `CLAUDE.md` 明令禁止）。
...
REFACTOR 契約（與 RED／GREEN 不同）：只重構、**不得改變任何行為**、全套測試維持綠燈、commit 訊息為 `refactor: <描述>`；完成後只跑 Stage 2，不跑 Stage 1（沒有 spec 可見的行為變動）。
...
- **upgrade_to_sonnet_when**：
  - task 觸及 §4／§5（React 元件、Playwright 假時鐘、hydration 時序）——這些牽涉框架行為而非單純函式。
  - 同一個 task 被任一 reviewer 退回 **2 次**。
  - Implementer 回報 `NEEDS_CONTEXT` 兩次仍無法收斂。
- **upgrade_to_opus_when**：
  - 同一個 task 被任一 reviewer 退回 **3 次**（見 Escalation）。
  - task 需要同時調整區間演算法與元件行為（本 change 的設計刻意避免此情況；若真的出現，代表 task 切分有誤，應先考慮拆 task）。
- **自我審查清單（commit 前自查）**：
...
- **必要的第一個動作**：先**複述**自己收到的 spec／test-plan 段落標題。若與待審的 task 不符，回報 `NEEDS_CONTEXT` 而**不要**開始審查（由 orchestrator 重組上下文後重新派工）。
- **審查清單**：
  - RED 測試是否確實對應 test-plan 指定的那個 `#### Scenario:`？
  - 該 requirement 中被本 task 涵蓋的 Scenario 是否**全部**有對應斷言？
...
- **同一個 reviewer 連續退回同一個 task 3 次** → 升級 Implementer 的模型（`haiku` → `sonnet` → `opus`）後重新派工。**SHALL NOT 以相同模型、相同上下文重複派工**。
...
- **發現 task 切分有誤**（一個 task 同時要求改演算法與改元件）→ 停止並回報，由 orchestrator 拆 task，不要硬做。
