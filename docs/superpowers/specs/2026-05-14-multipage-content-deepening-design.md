...
> 📌 **本設計未執行。**
> 文中宣告將拆為 3 個獨立 change，但 `openspec/changes/archive/` 內無任何對應 change，
> `nextjs-pickball/app/` 也不存在 learn / rules / equipment 等路由。
> 對應的 `plans/2026-05-14-multipage-phase-a.md` 全部 checkbox 皆未勾選。
> 保留為構想紀錄；若日後要做，請走 openspec change 流程重新提案。
